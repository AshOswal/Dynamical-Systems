function M = spm_DCM_lifpopsys_LC_int_spinit(P,M,InlineProgFlag)
...
while k < M.opt.N && r(k)>10*tol % integrate through time
    ...
    %ensure the point at the end of a full period is always computed
    if floor(M.opt.t(k)*M.opt.U.f0)>floor(M.opt.t(k-1)*M.opt.U.f0)
        dt = ceil(M.opt.t(k-1)*M.opt.U.f0)/M.opt.U.f0 - M.opt.t(k-1);
        M.opt.t(k) = M.opt.t(k-1) + dt;
    end
    
    
    for l = 1:M.nc % Compute recieved conductances
        Gout(l,:) = P.C(l).M; % P.A(l).M*squeeze(M.GV(:,l,k-1)) +;
    end
    U  = M.opt.U.u(M.opt.t(k)); % external inputs
    Gout = Gout + U;
    ...
    for l = 1:M.nc % Compute recieved conductances
        Gout(l,:) = P.C(l).M; %P.A(l).M*(squeeze(M.GV(:,l,k))) +; Midpoint
    end
    ...
    if ~(sum(D(:)) > tol)
        ...
        if M.opt.t(k) > .005 && ~round(1000*mod(M.opt.t(k),1/M.opt.U.f0)) %check only at the end of periods
        ...
        if ~isfield(M.opt,'pop'); M.opt.pop = 1; end;
        pop = M.opt.pop;
...
if abs(M.opt.T0-1/M.opt.U.f0)*M.opt.U.f0>.01*(1/M.opt.U.f0)
    fprintf('\n fundamental frequency badly estimated %f against %f\n',M.opt.U.f0,1/M.opt.T0)
end

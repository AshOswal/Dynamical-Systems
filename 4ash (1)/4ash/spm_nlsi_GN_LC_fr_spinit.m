function [Ep,Cp,Eh,F,dFdp,dFdpp,f] = spm_nlsi_GN_LC_fr_spinit(M,U,Y)
...
if ~isfield(M.opt,'vini'); M.opt.vini = 0;end;

criterion  = [0 0 0 0];
criterion2 = [0 0 0 0];

C.F   = -Inf;                                   % free energy
v     = M.opt.vini;                                     % log ascent rate
dFdh  = zeros(nh,1);
dFdhh = zeros(nh,nh);
%%
figDCM = figure(22);
...
%     revert = false;
    ...
    try
        ...
        [dfdp,f] = spm_DCM_lifpopsys_LC_adapstep_y_par_fr_spinit(Ep,M,V);

function [Ep,Cp,Eh,F,dFdp,dFdpp] = spm_nlsi_GN_LC_freq(M,U,Y)
...
C.F   = -Inf;                                   % free energy
v     = M.opt.vmin ;                                     % log ascent rate
dFdh  = zeros(nh,1);
dFdhh = zeros(nh,nh);

y(:,1:end-1) = Dfs(y(:,1:end-1)); 
%%
figDCM = figure(22);
set(figDCM,'Position',[1920 400 400 700])
...
%     k = 1;
    ...
    try
        ...
        [dfdp,f,flag] = spm_DCM_lifpopsys_LC_adapstep_y_par(Ep,M,V);
        dfdp     = reshape(spm_vec(dfdp),ns*nr,np);
        dfdp     = Dfs(dfdp); f.lc = Dfs(f.lc);
        % check for stability
        %------------------------------------------------------------------
...
%             E-Step: update
%             -------------------------------------------------------------
            dp       = spm_dx_constrained(dFdpp,dFdp,{v},dp0);
            disp(['dp0*dp = ' num2str((dp0*dp)'/norm(dp)/norm(dp0))])
            p        = p + dp;
            Ep       = spm_unvec(spm_vec(pE) + V*p(ip),pE);
            [dfdp,f,flag] = spm_DCM_lifpopsys_LC_adapstep_y_par(Ep,M,V);
            dfdp     =  reshape(spm_vec(dfdp),ns*nr,np);
    ...
    for k1 = 1:4
        ...
        hold on
        plot(log(abs(y(:,k1))),'r','LineWidth',2)
        plot(log(abs(f.lc(:,k1))),'Color',Cm(mod(k,6)+1,:))
        plot(log(abs(fp.lc(:,k1))),'.','Color',Cm(mod(k,6)+1,:),'MarkerSize',1)
        ylim([-5 20])
        hold off
    ...
    subplot(3,2,5)
    hold on
    plot(-(iDfs(y(:,2))),(iDfs(y(:,1))),'r','LineWidth',2)
    plot(-(iDfs(f.lc(:,2))),(iDfs(f.lc(:,1))),'Color',Cm(mod(k,6)+1,:))
    plot(-iDfs(fp.lc(:,2)),iDfs(fp.lc(:,1)),'.','Color',Cm(mod(k,6)+1,:),'MarkerSize',1)
    
    subplot(3,2,6)
    hold on
    plot(-(iDfs(y(:,4))),(iDfs(y(:,3))),'r','LineWidth',2)
    plot(-(iDfs(f.lc(:,4))),(iDfs(f.lc(:,3))),'Color',Cm(mod(k,6)+1,:))
    plot(-iDfs(fp.lc(:,4)),iDfs(fp.lc(:,3)),'.','Color',Cm(mod(k,6)+1,:),'MarkerSize',1)
%     hold off
    xlabel(['Ef = ' num2str(f.f(1)) '  dF = ' num2str(fp.f(1)) '  F = ' num2str(y(end))])
    drawnow
    ...
    save(['IterVar/Iter - ' num2str(k)], 'M','Ep','f')
end

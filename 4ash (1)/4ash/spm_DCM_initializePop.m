function [Ep] = spm_DCM_initializePop(M0,pop,Y)
...
YGI =  Y.y(:,3)/(M.opt.popCurrents(3,2) - P.P(1).Vg(2));
YGE =  Y.y(:,4)/(M.opt.popCurrents(4,2) - P.P(1).Vg(1));
YGI = YGI - min(YGI);
YGE = YGE - min(YGE);

function [Ep,Cp,Eh,F,dFdp,dFdpp] = spm_nlsi_GN_LC_freq1(M,U,Y)
...
C.F   = -Inf;                                   % free energy
v     = 4;%(M.opt.vmin + M.opt.vmax)/2;                                     % log ascent rate
dFdh  = zeros(nh,1);
dFdhh = zeros(nh,nh);
...
%     try
        ...
        dfdp     = reshape(spm_vec(dfdp),ns*nr,np);
            dfdp     = Dfs(dfdp); f.lc = Dfs(f.lc);f.fr = Dfs(f.fr);f.f = Dfs(f.f);
        % check for stability
        %------------------------------------------------------------------
...
%     end
    ...
    title(['Efr = ' num2str(f.fr(1,1)) '  dfr = ' num2str(fp.fr(1,1)) '  fr = ' num2str(y(1,end-1))])
...
%     hold off
    title(['Efr = ' num2str(f.fr(1,2)) '  dfr = ' num2str(fp.fr(1,2)) '  fr = ' num2str(y(1,end))])
    xlabel(['Ef = ' num2str(f.f(1)) '  df = ' num2str(fp.f(1)) '  f = ' num2str(y(1,end-2))])
    drawnow

...
p.S       = 0;%0.8;%5.5;
p.SN      = 100;%250;% normally we use 340;%105;%405
p.PhaseP  = 0;
p.Method  = 'S';
p.Plot    = 1;
cplot     = 0;
...
%             cfg.trial{i} = randn(size(cfg.trial{i}));
        ...
        %%
%         d = Dc(1,:,:,Dc.indtrial('granger_orig'))
%         figure;plot(Dc.frequencies, d)
        %%
        Gall(L,:,:,n)= G;         
        disp(num2str(n));
    end
...
for L = 1:6
    ...
    % power
    pow   = sqrt(collect(pows(L,:),'powspctrm'));
